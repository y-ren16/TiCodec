...
def get_dataset_filelist(a):
    with open(a.input_training_file, 'r') as f:
        ...
        # training_files = ["/data3/renyong/AcademiCodec/egs/HiFi-Codec-24k-320d/" + l.strip() for l in f]
    with open(a.input_validation_file, 'r') as f:
        validation_files = [l.strip() for l in f]
        # validation_files = ["/data3/renyong/AcademiCodec/egs/HiFi-Codec-24k-320d/" + l.strip() for l in f]
...
class MelDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if not self.fine_tuning:
            if self.split:
                if audio_bkp.size(1) >= self.segment_size:
                    max_audio_start = audio_bkp.size(1) - self.segment_size
                    audio_start = random.randint(0, max_audio_start)
                    audio = audio_bkp[:, audio_start:audio_start +
                                  self.segment_size]
                else:
                    audio = torch.nn.functional.pad(audio_bkp, (
                        0, self.segment_size - audio_bkp.size(1)), 'constant')
                if audio_bkp.size(1) >= self.segment_size:
                    max_audio_start = audio_bkp.size(1) - self.segment_size
                    audio_start_global = random.randint(0, max_audio_start)
                    audio_global = audio_bkp[:, audio_start_global:audio_start_global +
                                            self.segment_size]
                else:
                    audio_global = torch.nn.functional.pad(audio_bkp, (
                        0, self.segment_size - audio_bkp.size(1)), 'constant')
            mel = mel_spectrogram(
                audio,
                self.n_fft,
            ...
            if self.split:
                ...
                if audio_bkp.size(1) >= self.segment_size:
                    ...
                    audio = audio_bkp[:, mel_start * self.hop_size:(
        ...
        mel_loss = mel_spectrogram(
            ...
            center=False)
        if self.split:
            return (mel.squeeze(), audio.squeeze(0), filename, mel_loss.squeeze(), audio_global.squeeze(0))
        else:
